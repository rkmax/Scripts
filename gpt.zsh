...
choose_from_request_history() {
    ...
    result=("${(@f)$($history_script \
        | fzf --height 50% --reverse --border \
        --delimiter=$'\t' --with-nth=2.. --expect=tab \
        --header='GPT History (↑/↓: navigate, Enter: insert response, Tab: insert prompt)' \
        --preview "printf '\033[1;34mID:\033[0m %s\n\033[1;33mDate:\033[0m %s\n\n\033[1;34mPrompt:\033[0m\n' {1} {4}; ${(q)history_script} --get {1} --field prompt; printf '\n\n\033[1;32mResponse:\033[0m\n'; ${(q)history_script} --get {1} --field response" \
        --preview-window=up:wrap:60%:border)}")

    (( ${#result} < 2 )) && { zle redisplay; return 0 }

    key="${result[1]}"
    id="${result[2]%%$'\t'*}"
    [[ "$key" == tab ]] && field=prompt || field=response

    BUFFER="$($history_script --get "$id" --field "$field")"
    CURSOR=${#BUFFER}
    zle redisplay
}
